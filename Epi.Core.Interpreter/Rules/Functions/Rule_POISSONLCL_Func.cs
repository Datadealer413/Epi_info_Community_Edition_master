...
namespace Epi.Core.AnalysisInterpreter.Rules
{
    ...
    /// </summary>
    public partial class Rule_POISSONLCL_Func : AnalysisRule
    {
        private List<AnalysisRule> ParameterList = new List<AnalysisRule>();

        public Rule_POISSONLCL_Func(Rule_Context pContext, NonterminalToken pToken)
            : base(pContext)
        {
        ...
        public override object Execute()
        {
            ...
            if(double.TryParse(p1.ToString(), out param1))
            {
                if (param1 == 0.0)
                    result = 0.0;
                else if (param1 < 0.0 || param1 != (double)(int)param1)
                    result = Double.NaN;
                else
                {
                    result = LimitFromChiSq(0.975, 2 * param1);
                }
            }
